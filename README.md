# WebCam Controller

Windows PC의 웹캠과 마이크를 제어하고, 카메라 사용 여부를 실시간으로 감시하여 **텔레그램 · Discord 알림**을 보내는 보안 유틸리티입니다.  
추가로 시스템 오디오를 실시간으로 음성 인식하여 **자막 오버레이**로 표시하고 **DeepL로 번역**하는 기능도 제공합니다.

---

## 주요 기능

| 기능 | 설명 |
|------|------|
| 📷 카메라 제어 | 연결된 카메라 목록 표시, 클릭 한 번으로 활성화/비활성화 |
| 🎤 마이크 제어 | 연결된 마이크 목록 표시, 클릭 한 번으로 음소거/해제 |
| 👁️ 웹캠 감시 | 1~10초 간격으로 카메라 상태 자동 감시, 사용 중인 앱 이름 감지 |
| 🎙️ 마이크 감시 | 마이크 사용 시작·종료 시 알림 전송 (설정에서 활성화) |
| 📱 텔레그램 알림 | 이상 감지 시 스마트폰으로 즉시 알림 (6가지 이벤트) |
| 🔔 Discord 알림 | 웹훅 URL 입력만으로 Discord 채널에 알림 전송 |
| 🎬 화면 녹화 | 웹캠 사용 감지 시 자동 녹화 또는 수동 녹화 시작 (MJPEG AVI) |
| 💬 실시간 자막 | 시스템 오디오를 Vosk로 음성 인식 → 화면 오버레이로 자막 표시 |
| 🎨 오버레이 커스터마이징 | 자막 오버레이의 글꼴 크기, 불투명도, 배경색 설정 |
| 🌐 실시간 번역 | DeepL Free API로 인식된 음성을 자동 번역 (월 50만 자 무료) |
| 📋 이벤트 로그 | 날짜별 텍스트 파일에 이벤트 자동 기록, 날짜·종류별 뷰어 제공 |
| 🎬 녹화 파일 목록 | 저장된 녹화 파일 목록 확인, 더블클릭으로 재생 |
| 📝 자막 로그 | 음성 인식 원문·번역문을 날짜별 파일에 별도 기록 |
| 🖥️ 시스템 트레이 | 백그라운드 상주, 트레이 아이콘으로 조용히 동작 |
| 🔒 보안 저장 | 토큰 · URL · API 키를 Windows DPAPI로 암호화 저장 |
| 🛡️ 파일 변조 감지 | 실행 시마다 SHA-256 해시 검증 → 변조 시 즉시 알림 |
| 💾 저장 공간 경고 | 로그·영상 누적 용량 초과 및 디스크 여유 공간 부족 시 알림 |
| 📁 저장 폴더 지정 | 로그·자막·녹화 저장 위치를 원하는 드라이브로 변경 가능 (설치 시 또는 설정에서) |
| ⚙️ 시작 프로그램 | Windows 로그인 시 관리자 권한으로 자동 실행 |

---

## 감시 알림 이벤트

| 이벤트 | 알림 메시지 |
|--------|------------|
| 감시 시작 | 🟢 웹캠 감시 시작 |
| 카메라 장치 활성화 | ⚠️ 웹캠 활성화 감지! |
| 카메라 장치 비활성화 | 🔴 웹캠 비활성화 감지! |
| 앱이 카메라 사용 시작 (Zoom, Teams 등) | 📷 웹캠 사용 감지! (앱 이름 포함) |
| 앱이 카메라 사용 종료 | ⏹️ 웹캠 사용 종료 |
| 감시 종료 / 프로그램 종료 | 🔴 WebCam Monitor 종료 |

---

## 요구사항

- Windows 10 / 11 (x64)
- 관리자 권한 (카메라 장치 제어에 필요)
- [.NET 8 Desktop Runtime](https://dotnet.microsoft.com/download/dotnet/8.0) (미설치 시 설치 프로그램이 자동으로 다운로드)
- 인터넷 연결 (텔레그램 알림·DeepL 번역 사용 시)

---

## 설치

[Releases](../../releases) 페이지에서 `WebCamControl_Setup_v1.07.00.exe`를 다운로드하여 실행합니다.

설치 중 옵션:
- **Windows 시작 시 자동으로 실행** — 로그인 시 트레이에서 자동 감시 시작 (권장)
- **바탕화면에 바로가기 만들기**
- **데이터 저장 위치** — 기본값 또는 사용자 지정 드라이브 선택 (선택한 폴더 아래 `WebCamData\` 자동 생성)

> 설치 후 처음 실행 시 관리자 권한 요청 창이 표시됩니다. **예**를 눌러 허용해 주세요.

> **SmartScreen 경고가 표시되는 경우**  
> 설치 시 "Microsoft Defender SmartScreen에서 인식할 수 없는 앱" 경고가 뜰 수 있습니다.  
> 코드 서명 인증서가 없는 개인 개발 앱에서 발생하는 정상적인 경고입니다.  
> **"추가 정보" → "실행"** 을 클릭하면 설치가 진행됩니다.

---

## 사용 방법

1. **알림 설정** 버튼으로 텔레그램/Discord 설정 완료
2. **감시 시작** 버튼 클릭 → 버튼이 빨간색 **"감시 중지"** 로 바뀌면 감시 중
3. **자막 시작** 버튼 클릭 → 오버레이 창이 나타나 실시간 자막 표시
4. 창을 닫으면 트레이로 숨어서 계속 동작
5. 트레이 아이콘 더블클릭 → 창 복원 / 우클릭 → 메뉴

---

## 텔레그램 봇 준비 방법

1. 텔레그램에서 **@BotFather** 를 검색해 채팅 시작
2. `/newbot` 명령 입력 후 안내에 따라 봇 이름을 설정하면 **Bot Token** 발급
3. **@userinfobot** 을 검색해 `/start` 를 입력하면 본인의 **Chat ID** 확인
4. 앱에서 **알림 설정 → 텔레그램 탭** → Bot Token과 Chat ID 입력 후 **테스트 전송**으로 확인

---

## Discord 웹훅 설정 방법

1. 알림을 받을 채널 옆 톱니바퀴(⚙) 클릭 → **연동 탭 → 웹후크** 클릭
2. **새 웹후크** 버튼 클릭 → 이름 설정 → **웹후크 URL 복사**
3. 앱에서 **알림 설정 → Discord 탭** → URL 붙여넣기 → **Discord 알림 활성화** 체크 → **테스트 전송**으로 확인

---

## 감시 설정

**설정** 버튼에서 아래 항목을 조정할 수 있습니다.

| 항목 | 설명 |
|------|------|
| 감시 주기 (1~10초) | 카메라 상태를 확인하는 간격 (기본 3초) |
| 트레이 풍선 알림 표시 | PC 화면 우하단 팝업 on/off |
| 감시 시작·종료 알림 전송 | 감시 시작/중지/종료 시 알림 on/off |
| 마이크 사용 감지 알림 | 마이크 사용 시작·종료 시 알림 전송 활성화 |
| 날짜별 로그 저장 | 이벤트를 날짜별 텍스트 파일에 기록 |

로그 저장 위치: `%LocalAppData%\WebCamControl\logs\` (기본값, 설정에서 변경 가능)

---

## 데이터 저장 폴더

이벤트 로그, 자막 로그, 녹화 영상이 저장되는 위치를 원하는 드라이브/폴더로 변경할 수 있습니다.

**폴더 구조**
```
[지정 폴더]\
    WebCamData\logs\           ← 이벤트 로그
    WebCamData\subtitle_logs\  ← 자막 로그
    WebCamData\recordings\     ← 녹화 영상
```

기본값은 `%LocalAppData%\WebCamControl` 입니다.

**변경 방법**  
**설정 → 일반 탭 → 저장 폴더** 섹션에서 `[...]` 버튼으로 상위 폴더를 선택합니다.  
확인 클릭 후 기존 파일을 새 위치로 이동할지 선택할 수 있습니다.

> 디스크 여유 공간 10% 체크는 변경된 드라이브 기준으로 동작합니다.

---

## 저장 공간 경고

**설정 → 일반 탭 → 저장 공간 경고** 섹션에서 조정합니다.

| 항목 | 설명 |
|------|------|
| 용량 초과 시 경고 알림 | 누적 용량이 임계값을 초과하면 앱 시작 시 로그에 기록 |
| 로그 임계값 | 이벤트 로그 + 자막 로그 합산 기준 (MB) |
| 영상 임계값 | 녹화 파일 합산 기준 (GB) |

**디스크 여유 공간 자동 차단**  
앱 시작 시 저장 드라이브의 여유 공간을 확인하여 **10% 미만**이면:
- 트레이 풍선 알림 또는 팝업으로 경고
- 이벤트 로그, 자막 로그, 화면 녹화 저장 자동 비활성화
- 수동 녹화 버튼도 차단됨 (앱 재시작 시 재확인)

---

## 화면 녹화

웹캠이 사용되는 순간 자동 녹화하거나, **녹화 시작** 버튼으로 수동 녹화할 수 있습니다.

**설정 → 화면 녹화 탭**에서 아래 항목을 조정합니다.

| 항목 | 설명 |
|------|------|
| 웹캠 감지 시 자동 녹화 | 카메라 사용 감지 시 자동으로 녹화 시작 |
| 녹화 대상 모니터 | 여러 모니터 환경에서 녹화할 화면 선택 |
| 화질 | 낮음 5fps / 보통 10fps / 높음 15fps |

녹화 파일 저장 위치: `%LocalAppData%\WebCamControl\recordings\` (기본값, 설정에서 변경 가능)  
파일 형식: `yyyy-MM-dd_HH-mm-ss_태그_screen.avi`

**설정 → 화면 녹화 탭 → 녹화 파일 목록** 버튼으로 저장된 파일을 확인하고 더블클릭으로 바로 재생할 수 있습니다.

---

## 실시간 자막 / 번역 설정

### Vosk 음성 인식 모델 설치

Vosk는 인터넷 없이 동작하는 오프라인 음성 인식 엔진입니다. 모델 파일을 직접 다운로드해야 합니다.

1. [https://alphacephei.com/vosk/models](https://alphacephei.com/vosk/models) 에서 모델 다운로드
2. 압축을 원하는 폴더에 해제 (예: `C:\Vosk\vosk-model-small-ko-0.22`)
3. 앱에서 **설정 → 번역 탭 → 모델 추가** 버튼으로 경로 등록
4. 목록에서 사용할 모델을 클릭하면 파란색으로 활성화 — 여러 모델을 등록해 두고 즉시 전환 가능

**권장 모델**

| 언어 | 모델명 | 크기 | 특징 |
|------|--------|------|------|
| 한국어 | `vosk-model-small-ko-0.22` | 약 82 MB | 소형, 빠른 응답, 일상 대화에 적합 |
| 영어 (소형) | `vosk-model-small-en-us-0.15` | 약 40 MB | 매우 가볍고 빠름, 명확한 발음 권장 |
| 영어 (고품질) | `vosk-model-en-us-0.22` | 약 1.8 GB | 높은 인식률, 다양한 억양·어휘 지원 |

### DeepL API 키 발급

| 플랜 | 한도 | 속도 제한 | 실시간 번역 |
|------|------|-----------|------------|
| **Free** | 월 500,000자 무료, 신용카드 불필요 | 있음 | 응답 지연 가능 |
| **Pro** | 월정액 + 사용량 과금 | 없음 | 안정적, 권장 |

> 실시간 자막 번역을 안정적으로 사용하려면 **Pro 플랜**을 권장합니다.  
> Free 플랜은 속도 제한으로 인해 응답이 느리거나 오류가 발생할 수 있습니다.

**발급 방법**

1. [https://www.deepl.com/pro-api](https://www.deepl.com/pro-api) 접속 → 원하는 플랜 가입
2. 이메일 인증 완료 후 [https://www.deepl.com/account/summary](https://www.deepl.com/account/summary) 접속
3. **Authentication Key for DeepL API** 항목에서 키 복사  
   - Free 키: 끝에 `:fx` 붙음 (예: `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx:fx`)  
   - Pro 키: `:fx` 없음 (예: `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
4. 앱에서 **설정 → 번역 탭 → DeepL 번역 사용 체크 → API Key 입력**
5. **사용량 조회** 버튼으로 이번 달 남은 한도 확인 가능

> Free 플랜에서 월 500,000자를 모두 소진하면 번역이 자동으로 중단되고 자막만 계속 표시됩니다.  
> 다음 달 1일에 자동으로 초기화됩니다.

### 자막 오버레이 사용

1. 메인 화면 **자막 시작** 버튼 클릭 → 오버레이 창이 나타납니다
2. 오버레이는 마우스로 이동·크기 조절이 가능합니다
3. 하단(회색): 현재 인식 중인 텍스트 / 상단(누적): 확정된 자막
4. 번역 ON 시 원문(밝은 회색) + 번역(노란색)으로 표시
5. 번역 OFF 시 원문만 흰색으로 거의 실시간 표시

### 오버레이 커스터마이징

**설정 → 번역 탭 → 오버레이 설정** 에서 조정합니다.

| 항목 | 설명 |
|------|------|
| 글꼴 크기 | 자막 텍스트 크기 (8~24pt) |
| 불투명도 | 오버레이 창 투명도 (20~100%) |
| 배경색 | 오버레이 배경 색상 선택 |

자막 로그 저장 위치: `%LocalAppData%\WebCamControl\subtitle_logs\` (기본값, 설정에서 변경 가능)

---

## 시작 프로그램 관리

Windows **설정 → 앱 → 시작 프로그램** 에서 `WebCamControl` 항목을 켜거나 끌 수 있습니다.  
처음 실행 시 등록하지 않았더라도 위 경로에서 언제든지 등록할 수 있습니다.

---

## 보안 기능

### DPAPI 암호화
Bot Token · Chat ID · Discord Webhook URL · DeepL API 키는 Windows DPAPI(현재 사용자 범위)로 암호화하여 저장합니다.
동일 PC · 동일 계정 외에는 복호화가 불가능합니다.

### 파일 무결성 검증
실행할 때마다 exe 파일의 SHA-256 해시를 HKLM 레지스트리 저장값과 비교합니다.
불일치(변조) 감지 시 텔레그램 알림 전송 후 프로그램을 종료합니다.

---

## 빌드

```powershell
# 개발 빌드
dotnet build

# 릴리즈 빌드 (프레임워크 의존, 단일 파일 — .NET 8 설치 필요)
dotnet publish -c Release -r win-x64 --self-contained false `
  -p:PublishSingleFile=true

# 인스톨러 (Inno Setup 7 필요)
& "C:\Program Files\Inno Setup 7\ISCC.exe" WebCamControl.iss
```

---

## 기술 스택

| 항목 | 내용 |
|------|------|
| 언어 | C# (.NET 8, WinForms) |
| 카메라 제어 | Windows SetupAPI (P/Invoke) |
| 마이크 제어 | Windows CoreAudio API (P/Invoke) |
| 카메라 사용 감지 | Windows Registry (CapabilityAccessManager/webcam) |
| 마이크 사용 감지 | Windows Registry (CapabilityAccessManager/microphone) |
| 화면 녹화 | GDI CopyFromScreen + SharpAvi (MJPEG AVI) |
| 음성 인식 | Vosk 0.3.38 (오프라인 STT) |
| 번역 | DeepL Free API (HttpClient) |
| 오디오 캡처 | NAudio 2.2.1 (WasapiLoopbackCapture) |
| 텔레그램 알림 | Telegram Bot API (HttpClient) |
| Discord 알림 | Discord Webhook (HttpClient) |
| 이벤트 로그 | 날짜별 텍스트 파일 (기본: AppData\Local\WebCamControl\logs) |
| 자막 로그 | 날짜별 텍스트 파일 (기본: AppData\Local\WebCamControl\subtitle_logs) |
| 저장 경로 관리 | 데이터 루트 폴더 변경 + 기존 파일 이동 지원 |
| 설정 암호화 | Windows DPAPI (ProtectedData) |
| 파일 무결성 검증 | SHA-256 + DPAPI (HKLM 레지스트리) |
| 시작 프로그램 등록 | Windows Task Scheduler (schtasks) |
| 인스톨러 | Inno Setup 7 |

---

## 릴리즈 히스토리

| 버전 | 주요 변경 사항 |
|------|--------------|
| **v1.07.00** | 마이크 사용 감지 알림, 자막 오버레이 커스터마이징 (글꼴 크기·불투명도·배경색), 녹화 파일 목록 UI (더블클릭 재생), 로그 뷰어 날짜 선택 + 감시/자막 로그 전환 |
| **v1.06.21** | 데이터 저장 폴더 지정 기능 (설치 시 + 설정에서 변경, WebCamData 하위 폴더 자동 생성, 기존 파일 이동 지원), 설정창 스크롤 지원, 설치 시 경로 반영 버그 수정, 녹화 시작 실패 시 이벤트 로그 기록 |
| **v1.06.11** | 웹캠 사용 앱 이름 자동 감지 표시, 저장 공간 경고 (누적 용량 임계값 + 디스크 10% 미만 시 저장 차단) |
| **v1.06.01** | 실시간 자막 오버레이 (Vosk), DeepL 번역 연동, 다중 Vosk 모델 지원, 카메라 감지 디바운스, 앱 종료 크래시 수정 |
| **v1.05** | 화면 녹화 (자동/수동, MJPEG AVI), 단일 인스턴스 체크 |
| **v1.04** | 감시 설정 창 (주기·알림·로그), 이벤트 로그 뷰어, 파일 무결성 SHA-256 검증 |
| **v1.03** | Discord 웹훅 알림 추가, 알림 설정 UI 통합 |
| **v1.02** | 최초 릴리즈 — 카메라/마이크 제어, 웹캠 감시, 텔레그램 알림, 시스템 트레이 |

---

## 라이선스

개인 사용 목적으로 제작되었습니다.

---

제작자: jaeho · jaeho9697@gmail.com
